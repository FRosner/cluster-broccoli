...
class CouchDBInstanceStorageSpec extends Specification {
  ...
  "Creating the CouchDB instance storage" should {
    ...
    "succeed if the database already exists" in {
      Server.withRouter() {
        case GET(p"/broccoli_instances") =>
          Action {
            Results.Ok
          }
        case PUT(p"/_config/broccoli/broccoli_instances") =>
          Action {
            Results.Ok(JsString(""))
          }
      } { implicit port =>
        WsTestClient.withClient { client =>
          new CouchDBInstanceStorage(s"http://localhost:$port", "broccoli_instances", client) should beAnInstanceOf[
            CouchDBInstanceStorage]
        }
      }
    }
    ...
    "fail if the database does not exist but cannot be created" in {
      Server.withRouter() {
        ...
        WsTestClient.withClient { client =>
          new CouchDBInstanceStorage(s"http://localhost:$port", "broccoli_instances", client) should throwA[
    ...
    "fail if the database cannot be reached" in {
      WsTestClient.withClient { client =>
        new CouchDBInstanceStorage(s"http://localhost:43643", "", client) should throwA[ConnectException]
      }
    }
    ...
    "lock the database" in {
      ...
      Server.withRouter() {
        ...
        WsTestClient.withClient { client =>
          new CouchDBInstanceStorage(s"http://localhost:$port", "broccoli_instances", client)
          locked === true
        }
      }
    }
  ...
  "Closing the CouchDBInstanceStorage" should {
    ...
    "release the lock" in {
      ...
      Server.withRouter() {
        ...
        case DELETE(p"/_config/broccoli/broccoli_instances") =>
          Action {
            released = true
            Results.Ok
          }
        ...
        WsTestClient.withClient { client =>
          new CouchDBInstanceStorage(s"http://localhost:$port", "broccoli_instances", client).close()
          released === true
        }
      }
    }
  ...
  "Reading all instances" should {
    ...
    "work" in {
      Server.withRouter() {
        ...
        case GET(p"/broccoli_instances/_all_docs") =>
          Action {
        ...
        WsTestClient.withClient { client =>
          val storage = new CouchDBInstanceStorage(s"http://localhost:$port", "broccoli_instances", client)
    ...
    "fail if an instance ID does not match the document ID" in {
      Server.withRouter() {
        ...
        case GET(p"/broccoli_instances/_all_docs") =>
          Action {
            val docJson =
              JsObject(Json.toJson(instance).as[JsObject].value.updated("_id", JsString(instance.id + "xxx")))
            Results.Ok(
              Json.obj(
                "offset" -> 0,
                "rows" -> Json.arr(
  ...
  "Reading an instance" should {
    ...
    "work" in {
      Server.withRouter() {
        ...
        case GET(p"/broccoli_instances/prefix-id") =>
          Action {
    ...
    "fail if the document is not a valid instance JSON" in {
      Server.withRouter() {
        ...
        case GET(p"/broccoli_instances/prefix-id") =>
          Action {
            Results.Ok(
              JsObject(Json.toJson(instance).as[JsObject].value.updated("_id", JsString(instance.id)).-("template")))
          }
        case PUT(p"/_config/broccoli/broccoli_instances") =>
          Action {
            Results.Ok(JsString(""))
  ...
  "Writing an instance" should {
    ...
    "work for newly created instances" in {
      Server.withRouter() {
        ...
        case PUT(p"/broccoli_instances/prefix-id") =>
          Action {
            Results.Created
          }
        case PUT(p"/_config/broccoli/broccoli_instances") =>
    ...
    "work for instances overwriting another one" in {
      ...
      Server.withRouter() {
        ...
        case PUT(p"/broccoli_instances/prefix-id") =>
          Action { request =>
            require(request.body.asJson.get.as[JsObject].value("_rev") == JsString(revision),
                    "Revision needs to be put as well.")
            Results.Created
          }
        case PUT(p"/_config/broccoli/broccoli_instances") =>
          Action {
  ...
  "Deleting an instance" should {
    ...
    "work" in {
      ...
      Server.withRouter() {
        ...
        case DELETE(p"/broccoli_instances/prefix-id") =>
          Action {
            Results.Ok
          }
        case PUT(p"/_config/broccoli/broccoli_instances") =>
