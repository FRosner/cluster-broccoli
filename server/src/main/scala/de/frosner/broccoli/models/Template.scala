...
object Template {
  ...
  implicit def paramInfoMapWrites(implicit account: Account): Writes[Map[String, ParameterInfo]] = new Writes[Map[String, ParameterInfo]] {
    def writes(map: Map[String, ParameterInfo]): JsValue =
      Json.obj(map.map{case (s, paramInfo) =>
